...
if (adminToken === "dev-admin") {
  console.warn("⚠️  Используется дефолтный ADMIN_TOKEN. В продакшене задайте переменную окружения ADMIN_TOKEN!");
}
  ...
  try {
    ...
    if (url.pathname === "/api/bookings" && req.method === "POST") {
      ...
      try {
        ...
        if (taken) {
          db.exec("ROLLBACK");
          return json(res, 409, { error: "Это время уже заняли. Выберите другое окно." });
        }
    ...
    if (!res.headersSent) {
      try {
        json(res, 500, { error: "На сервере что-то пошло не так." });
      } catch {
        res.end();
      }
    }
...
function seedDatabase() {
  ...
  const upsertSlot = db.prepare(`
    INSERT INTO slots (id, starts_at, duration_minutes, active)
    VALUES (?, ?, ?, 1)
    ON CONFLICT(id) DO UPDATE SET
  ...
  for (let offset = 0; addedDays < schedule.daysToGenerate && offset < 60; offset += 1) {
    ...
    times.forEach((time) => {
      ...
      upsertSlot.run(id, `${day}T${time}:00`, schedule.durationMinutes || 60);
    });
    addedDays += 1;
  }
}
...
function validateBooking(body) {
  ...
  const privacyAccepted =
    ...
    body.privacyAccepted === "true" ||
    body.privacyAccepted === "yes" ||
    body.privacyAccepted === 1 ||
    body.privacyAccepted === "1";
